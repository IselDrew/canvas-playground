function Canvas(tile, mapSize) {
  ...
  document.body.querySelector(".snake").appendChild(this.canvas)
  this.resizeCanvas()
...
function Snake(tile, mapSize, gameSpeed) {
  ...
  this.canvas = new Canvas(32, [15, 10])
...
Snake.prototype = {
  ...
  isBerryCoordsValid: function() {
    for (let i = 0; i < this.snake.length; i++) {
      if (
        this.snake[i].x === this.berry.x &&
        this.snake[i].y === this.berry.y
      ) {
        return false
      }
    }
  ...
  bordersCollision: function(head) {
    if (
      head.x < 0 ||
      head.x >= this.mapWidth ||
      head.y < 0 ||
      head.y >= this.mapHeight
    ) {
      return true
    }
  },
  ...
  gameOver: function() {
    ...
    // this.restart = confirm("Your score is " + this.scoreCounter())
...
;(function() {
  console.log("TODO:\nAdd restart feature")
  const snake = new Snake(32, [15, 10], 200)
})()
